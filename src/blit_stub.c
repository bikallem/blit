...
MOONBIT_FFI_EXPORT void bikallem_blit_blit_fixed_array(
...
MOONBIT_FFI_EXPORT void bikallem_blit_fill_bytes(
    moonbit_bytes_t dst, int32_t dst_off,
...
MOONBIT_FFI_EXPORT int32_t bikallem_blit_match_length(
...
// Subtract `delta` from each element of a FixedArray[Int] (int32 array),
// clamping to `floor` (typically -1).
MOONBIT_FFI_EXPORT void bikallem_blit_shift_int_array(
    int32_t *arr, int32_t len, int32_t delta, int32_t floor) {
  for (int32_t i = 0; i < len; i++) {
    int32_t v = arr[i] - delta;
...
MOONBIT_FFI_EXPORT moonbit_bytes_t bikallem_blit_make_uninit(
    int32_t len) {
...
MOONBIT_FFI_EXPORT int64_t *bikallem_blit_make_uninit_int(
    int32_t len) {
